...
namespace FrozenThrone
{
    ...
    int Lion::encode_bgr(uint8_t * bgr_ptr, cv::Size size, uint8_t*& image_compressed, int& image_compressed_size)
    {
...
//for idct we must add some memory - it rounds up the block count, computes all and the extra bytes are omitted
        ...
        if (gpujpeg_encoder_encode(encoder_, &param_, &param_image_, &encoder_input_, &image_compressed, &image_compressed_size) != 0 ) {
        ...
        // Save image
        // if ( gpujpeg_image_save_to_file("output_gpujpeg.jpg", image_compressed, image_compressed_size) != 0 ) {
